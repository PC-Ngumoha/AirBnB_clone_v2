...
def deploy():
    """Performs the entire process of deploying the static files to servers."""
...
def do_clean(number=0):
    """Eliminates all useless archive files on the local and remote environment
    """
    archives = local('ls versions/', capture=True)
    if number == 0 or number == 1:
        number_to_delete = len(archives) - 1
    elif number > 1:
        number_to_delete = len(archives) - number
    for i in range(0, number_to_delete):
        local('rm -f versions/{}'.format(archives[i]))
        run('rm -rf /data/web_static/releases/{}'
            .format(archives[i].split('.')[0]))
